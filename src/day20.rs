                ...
                letter if letter.is_ascii_uppercase() => {
                    for (nx, ny) in get_successors(x, y, w, h) {
                        if maze_matrix[ny][nx].is_ascii_uppercase() {
                            ...
                            } else if portals.contains_key(&name) {
                                maze.entry(pos)
                                    .and_modify(|cell| {
                                        cell.portal = Some((portals[&name].0, !portals[&name].1))
                                    })
                                    .or_insert_with(|| {
                                        Cell::new(
                                            pos,
                                            true,
                                            Some((portals[&name].0, !portals[&name].1)),
                                            0,
                                        )
                                    });
                                maze.entry(portals[&name].0)
                                    .and_modify(|cell| cell.portal = Some((pos, portals[&name].1)))
                                    .or_insert_with(|| {
                                        Cell::new(
                                            portals[&name].0,
                                            true,
                                            Some((pos, portals[&name].1)),
                                            0,
                                        )
                                    });
                            } else {
