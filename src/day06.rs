...
pub fn solve(input: String) -> Solution<usize, usize> {
    ...
    let p2 = solve_part2(&undirected, *ids.get("YOU").unwrap(), *ids.get("SAN").unwrap()).unwrap();
    Solution::new(p1, p2)
}
...
fn prepare_maps(input: String) -> (HashMap<String, usize>, Relations, Relations) {
    ...
    for line in input.lines() {
        ...
        let center = *ids.entry(String::from(line_iter.next().unwrap()))
            .or_insert_with(|| { id += 1; id });
        let orbiter = *ids.entry(String::from(line_iter.next().unwrap()))
            .or_insert_with(|| { id += 1; id });
        directed.entry(center).or_insert_with(|| Vec::new()).push(orbiter);
        undirected.entry(center).or_insert_with(|| Vec::new()).push(orbiter);
        undirected.entry(orbiter).or_insert_with(|| Vec::new()).push(center);
    }
